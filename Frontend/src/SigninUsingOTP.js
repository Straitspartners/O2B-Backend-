...
const OtpContainer = () => {
  ...
  const requestOtp = async () => {
    try {
      const response = await axios.post('http://192.168.31.111:8000/login', { identifier, login_type: 'otp' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.31.111:8000/login', userData);
