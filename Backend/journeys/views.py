...
# # from .utils import fetch_airports 
...
#             serializer = JourneyPreferencesSerializer(journey_preferences)
...
#         try:
#             journey_preferences = JourneyPreferences.objects.get(user=user)
#             serializer = JourneyPreferencesSerializer(journey_preferences, data=request.data, partial=True)
#             if serializer.is_valid():
#                 serializer.save()
#                 return Response(serializer.data, status=status.HTTP_200_OK)
#             return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
# class PlacesToVisitView(APIView):
#     permission_classes = [IsAuthenticated]

#     def get(self, request):
#         user = request.user
#         # Filter PlacesToVisit through JourneyPreferences related to the user
#         journey_preferences = JourneyPreferences.objects.filter(user=user)
#         places = PlacesToVisit.objects.filter(journey_preferences__in=journey_preferences)
#         serializer = PlacesToVisitSerializer(places, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = PlacesToVisitSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#         user = request.user
#         try:
#             # Validate if the user has a related JourneyPreferences
#             journey_preferences = JourneyPreferences.objects.get(user=user)
#             # Get the PlacesToVisit entry linked to the user's JourneyPreferences
#             place_to_visit = PlacesToVisit.objects.get(journey_preferences=journey_preferences)
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
#         except PlacesToVisit.DoesNotExist:
#             return Response({"error": "Places to visit not found."}, status=status.HTTP_404_NOT_FOUND)

#         serializer = PlacesToVisitSerializer(place_to_visit, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_200_OK)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
# class StayPreferencesView(APIView):
#     permission_classes = [IsAuthenticated]
...
#         # Filter PlacesToVisit through JourneyPreferences related to the user
#         journey_preferences = JourneyPreferences.objects.filter(user=user)
#         stays = StayPreferences.objects.filter(journey_preferences__in=journey_preferences)
#         serializer = StayPreferencesSerializer(stays, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = StayPreferencesSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
#     # In your view, for the PUT request
#     def put(self, request):
#         user = request.user
#         try:
#         # First, get the JourneyPreferences for the user
#             journey_preferences = JourneyPreferences.objects.get(user=user)
        
#         # Then, get StayPreferences using the related JourneyPreferences
#             stay_preferences = StayPreferences.objects.get(journey_preferences=journey_preferences)
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
#         except StayPreferences.DoesNotExist:
#             return Response({"error": "Stay preferences not found."}, status=status.HTTP_404_NOT_FOUND)

#     # Now proceed with updating the StayPreferences as before
#         serializer = StayPreferencesSerializer(stay_preferences, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_200_OK)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#     def get(self, request):
#         user = request.user
#         details = TravelDetails.objects.filter(user=user)
#         serializer = TravelDetailsSerializer(details, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = TravelDetailsSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#         user = request.user
#         try:
#             details = TravelDetails.objects.get(user=user)
#             serializer = TravelDetailsSerializer(details, data=request.data, partial=True)
#             if serializer.is_valid():
#                 serializer.save()
#                 return Response(serializer.data, status=status.HTTP_200_OK)
#             return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
#         except TravelDetails.DoesNotExist:
#             return Response({"error": "Travel Details not found."}, status=status.HTTP_404_NOT_FOUND)
...
#     def get(self, request):
#         user = request.user
#         preferences = FoodPreferences.objects.filter(user=user)
#         serializer = FoodPreferencesSerializer(preferences, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = FoodPreferencesSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
#         except FoodPreferences.DoesNotExist:
#             return Response({"error": "Stay preferences not found."}, status=status.HTTP_404_NOT_FOUND)

#     # Now proceed with updating the StayPreferences as before
#         serializer = FoodPreferencesSerializer(food_preferences, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_200_OK)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
# class VehiclePreferencesView(APIView):
#     permission_classes = [IsAuthenticated]
...
#         # Filter PlacesToVisit through JourneyPreferences related to the user
#         journey_preferences = JourneyPreferences.objects.filter(user=user)
#         vehicles = VehiclePreferences.objects.filter(journey_preferences__in=journey_preferences)
#         serializer = VehiclePreferencesSerializer(vehicles, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = VehiclePreferencesSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#         # Then, get VehiclePreferences using the related JourneyPreferences
#             vehicles = VehiclePreferences.objects.get(journey_preferences=journey_preferences)
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
#         except VehiclePreferences.DoesNotExist:
#             return Response({"error": "Stay preferences not found."}, status=status.HTTP_404_NOT_FOUND)

#     # Now proceed with updating the VehiclePreferences as before
#         serializer = VehiclePreferencesSerializer(vehicles, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_200_OK)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

# class TourGuidePreferencesView(APIView):
#     permission_classes = [IsAuthenticated]
...
#         # Filter PlacesToVisit through JourneyPreferences related to the user
#         journey_preferences = JourneyPreferences.objects.filter(user=user)
#         tour = TourGuidePreferences.objects.filter(journey_preferences__in=journey_preferences)
#         serializer = TourGuidePreferencesSerializer(tour, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = TourGuidePreferencesSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#         # Then, get VehiclePreferences using the related JourneyPreferences
#             language = TourGuidePreferences.objects.get(journey_preferences=journey_preferences)
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
#         except TourGuidePreferences.DoesNotExist:
#             return Response({"error": "Stay preferences not found."}, status=status.HTTP_404_NOT_FOUND)

#     # Now proceed with updating the VehiclePreferences as before
#         serializer = TourGuidePreferencesSerializer(language, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_200_OK)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

# class CateringOrChefView(APIView):
#     permission_classes = [IsAuthenticated]
...
#         # Filter PlacesToVisit through JourneyPreferences related to the user
#         journey_preferences = JourneyPreferences.objects.filter(user=user)
#         chef = CateringOrChef.objects.filter(journey_preferences__in=journey_preferences)
#         serializer = CateringOrChefSerializer(chef, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = CateringOrChefSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#         # Then, get VehiclePreferences using the related JourneyPreferences
#             chef = CateringOrChef.objects.get(journey_preferences=journey_preferences)
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
#         except CateringOrChef.DoesNotExist:
#             return Response({"error": "Stay preferences not found."}, status=status.HTTP_404_NOT_FOUND)

#     # Now proceed with updating the VehiclePreferences as before
#         serializer = CateringOrChefSerializer(chef, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_200_OK)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

# class PaperworkAssistanceView(APIView):
...
#         # Get the PaperworkAssistance for the specific journey_preferences_id
#         try:
#             paperwork = PaperworkAssistance.objects.get(
#                 journey_preferences__user=user,
#                 journey_preferences__id=journey_preferences_id
#             )
#             serializer = PaperworkAssistanceSerializer(paperwork)
#             return Response(serializer.data, status=status.HTTP_200_OK)

#         except PaperworkAssistance.DoesNotExist:
#             return Response({"error": "Paperwork assistance not found."}, status=status.HTTP_404_NOT_FOUND)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = PaperworkAssistanceSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#         try:
#         # Fetch the specific PaperworkAssistance instance
#             paperwork = PaperworkAssistance.objects.get(
#             journey_preferences__user=user,
#             journey_preferences__id=journey_preferences_id
#         )
...
#         serializer = PaperworkAssistanceSerializer(paperwork, data=request.data, partial=True)
...
#         # Filter PlacesToVisit through JourneyPreferences related to the user
#         journey_preferences = JourneyPreferences.objects.filter(user=user)
#         vendor = Vendor.objects.filter(journey_preferences__in=journey_preferences)
...
#         # Then, get VehiclePreferences using the related JourneyPreferences
#             vendors= Vendor.objects.get(journey_preferences=journey_preferences)
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
...
# class ExtraRequestsView(APIView):
#     permission_classes = [IsAuthenticated]
...
#         # Filter PlacesToVisit through JourneyPreferences related to the user
#         journey_preferences = JourneyPreferences.objects.filter(user=user)
#         extra = ExtraRequests.objects.filter(journey_preferences__in=journey_preferences)
#         serializer = ExtraRequestsSerializer(extra, many=True)
#         return Response(serializer.data, status=status.HTTP_200_OK)

#     def post(self, request):
#         data = request.data
#         data['user'] = request.user.id
#         serializer = ExtraRequestsSerializer(data=data)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_201_CREATED)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
#             # Then, get the ExtraRequests instance related to this JourneyPreferences
#             extra_request = ExtraRequests.objects.get(journey_preferences=journey_preferences)
#         except JourneyPreferences.DoesNotExist:
#             return Response({"error": "Journey preferences not found for the user."}, status=status.HTTP_404_NOT_FOUND)
#         except ExtraRequests.DoesNotExist:
#             return Response({"error": "Extra requests not found for this journey preferences."}, status=status.HTTP_404_NOT_FOUND)

#         # Proceed with updating the ExtraRequests instance with the new data
#         serializer = ExtraRequestsSerializer(extra_request, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data, status=status.HTTP_200_OK)
#         return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
